...
Development App Attest is green. The UI/UX item remains red and has expanded
into Product Experience V2. The Phase 8 device steps above (section 4a and the
strengthened section 5 checks) were anchored to `f92eda8`; the V2 exit gate
re-runs them against the V2 candidate. M3B unlocks only after the complete V2
simulator and physical-device exit gate is green. Phase 9 and M3B/M3C stay
explicitly blocked until then; this checklist prepares the steps and evidence
format, it does not itself close the gate.
