...
**Correction (same day):** the Task 15.1 closure report said the build had only the two AppIntents notices. That was wrong. `MePreferredBagSelection`'s `Binding(get:set:)` closures captured the non-Sendable `PackingPreferenceRecord` in `@Sendable` closures and produced two Swift warnings. The focused 15.1 runs showed them. The full run was an incremental build that never recompiled `MeView.swift`, so its log hid them. Fixed by binding through a computed `singleSelectPreferredBagRaw` property via `Bindable(prefs)`. A from-scratch build now reports zero Swift compile warnings.
