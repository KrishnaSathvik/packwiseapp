...
- **Not implemented yet:** bags still reach the engine through temporary singleton accessors (`TripContext.bagType`, `TripRecord.bagType`); a multi-bag selection resolves to `.notSure` (no bag constraint), never a primary bag, until Task 5. Setup, Trip Detail, and Packing List still read `TripRecord.tripType` for presentation until Tasks 8/10/13, and setup and Me still edit one trip type and one bag until Task 8, so no shipped screen can create a multi-value trip yet.
